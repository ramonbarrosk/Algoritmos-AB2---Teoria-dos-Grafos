...
int* dijkstra(Grafo g, int orig, int *distancia){
	int V = g.getVertice();
...
void ajuda(){
    printf("Algoritmo de Dijkstra para caminho com o menor custo:\n"
    ...
    output_file.open(output_name);
     ...
     if(f){
        ...
        if (in && l){
						int v = g.getVertice();
						int arr[v];
            int* distancia = dijkstra(g, source, arr);
            if (!o) printf("Distância do vértice %d para o vértice %d = %d\n", source, dest, distancia[dest]);
                ...
                output_file << "Distância do vértice " << source << " para o vértice: " << dest <<" = " << distancia[dest] << endl; 
					...
					int v = g.getVertice();
					int arr[v];
          int* dist = dijkstra(g, source, arr);
